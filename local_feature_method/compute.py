...
def generate_index(number, table):
...
def get_query_list():
    ...
    test_path = os.path.join(datasets_path, "Testing", str(150), "query_drone")
    test_list = glob.glob(os.path.join(test_path,"*"))
    for i in range(len(test_list)):
        test_list[i] = test_list[i][-4:]
    test_list = sorted(test_list, key=lambda x: int(re.findall("[0-9]+", x)[0]))
    return test_list
    # table['query'] = test_list
    # pd.DataFrame(table).to_csv("csv_dir" + os.sep + "query_list.csv")
    # print(test_list)
...
if __name__ == '__main__':
    ...
    for i in query:
        ap_tmp, CMC_tmp = generate_index(i, df)
        if CMC_tmp[0] == -1:
            continue
        CMC += CMC_tmp
        ap += ap_tmp
