...
from LPN_model import two_view_net
from LPN.test_and_evaluate import eval_and_test
import os
...
def train():
    ...
    model = model_.model_dict[model_name](classes, drop_rate, share_weight=False, block=block)
    model = model.cuda()
    ...
    for epoch in range(num_epochs):
        ...
        for data1, data2 in zip(data_loader["satellite_train"], data_loader["drone_train"]):
            ...
            preds1, loss1 = one_LPN_output(output1, label1, criterion, block)
            preds2, loss2 = one_LPN_output(output2, label2, criterion, block)
        ...
        if drone_acc > 0.98 and satellite_acc > 0.98:
