...
def extract_feature(model, dataloaders, block, view_index=1):
...
def eval_and_test():
    ...
    #     transforms.Resize((get_yaml_value("image_size"), get_yaml_value("image_size")), interpolation=3),
    #     transforms.ToTensor(),
    #     transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    # ])
    #
    # image_datasets = {x: datasets.ImageFolder(os.path.join("/media/data1/University-Release/University-Release/test", x), data_transforms) for x in
    #                   ['gallery_satellite', 'gallery_drone', 'query_satellite', 'query_drone']}
    # data_loader = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=get_yaml_value("batch_size"),
    ...
    if os.path.exists(os.path.join(get_yaml_value("weight_save_path"),
        ...
        for query in ['drone', 'satellite']:
            for seq in range(-2, 0):
                ...
                with torch.no_grad():
                    since = time.time()
                    query_feature = extract_feature(model, data_loader[query_name], block, which_query)
                    gallery_feature = extract_feature(model, data_loader[gallery_name], block, which_gallery)
                    ...
                    scipy.io.savemat('LPN_pytorch_result.mat', result)
                ...
                result = scipy.io.loadmat("LPN_pytorch_result.mat")
